(function() {
    ...
    function account(url) {
        ...
        if (connection_source !== 'hdpoisk') {
            if (url.indexOf('token=') == -1) {
                var t = '';
                if (t != '') url = Lampa.Utils.addUrlComponent(url, 'token=');
            }
        ...
        this.initialize = function() {
            ...
            filter.onSelect = function(type, a, b) {
                if (type == 'filter') {
                    if (a.stype == 'connection') {
                        ...
                        if (a.stype == 'voice') { choice.voice_name = filter_find.voice[b.index].title; choice.voice_url = filter_find.voice[b.index].url; }
                        choice[a.stype] = b.index;
                        _this.saveChoice(choice);
                        _this.reset();
                        _this.request(filter_find[a.stype][b.index].url);
                        setTimeout(Lampa.Select.close, 10);
                    }
                } else if (type == 'sort') {
                    Lampa.Select.close();
            ...
            if (object.balanser) {
                ...
                return network["native"](account(object.url.replace('rjson=', 'nojson=')), this.parse.bind(this), function() {
                    ...
                    _this.empty();
                }, false, { dataType: 'text', headers: { 'X-Kit-AesGcm': Lampa.Storage.get('aesgcmkey', '') } });
            }
        ...
        this.createSource = function() {
            ...
            return new Promise(function(resolve, reject) {
                ...
                network.silent(account(url), function(json) {
                    ...
                    } else { _this.startSource(json).then(resolve)["catch"](reject); }
                }, reject, false, { headers: { 'X-Kit-AesGcm': Lampa.Storage.get('aesgcmkey', '') } });
            });
        };
    ...
    function startPlugin() {
        ...
        var manifst = {
            ...
            onContextMenu: function() { return { name: 'Смотреть онлайн' }; },
        ...
        Lampa.Listener.follow('full', function(e) {
            if (e.type == 'complite') {
                var btn = $('<div class="full-start__button selector view--online lampac--button"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M8 5v14l11-7z"/></svg><span>Смотреть онлайн</span></div>');
